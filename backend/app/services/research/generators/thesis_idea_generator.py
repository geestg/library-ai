...
from app.services.research.generators.thesis_prompts import (
...
from app.services.research.analysis.multi_doc_synthesizer import (
...
def sanitize_and_enhance_ideas(ideas: str) -> str:
    ...
    # Clean and separate section headers with double newlines and bolding
    section_headers = [
        "Problem",
        "Research Gap",
        "Solusi & Kebaruan",
    ...
    for h in section_headers:
        # Match 'ProblemSistem' or '**Problem**Sistem' or 'Problem: Sistem' or '### Problem'
        pattern = r"(?mi)^\s*(?:#{1,3}\s*)?(?:\*{0,2})" + re.escape(h) + r"(?:\*{0,2}):?\s*(\S[^\n\r]*)"
        ideas = re.sub(pattern, r"\n\n**" + h + r"**\n\1", ideas)
