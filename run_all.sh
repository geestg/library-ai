...
if curl -s http://127.0.0.1:11435/v1/models > /dev/null 2>&1; then
    ...
    nohup python3 -m vllm.entrypoints.openai.api_server \
        ...
        --host 0.0.0.0 \
        --port 11435 \
        --gpu-memory-utilization 0.70 > /workspace/slm_11435.log 2>&1 &
fi
...
if curl -s http://127.0.0.1:11436/v1/models > /dev/null 2>&1; then
    ...
    echo "ℹ️ [2/4] Port 11436 (Llama 3.3 70B) tidak aktif. DELBot fokus menggunakan Qwen3-30B-MoE di Port 11435."
fi
...
echo "• Primary LLM (Qwen3-30B-MoE)  : Port 11435"
